 ...
 */
function main() {
  const bookingRows = prepareUpdateFile("booking-links.csv", constants.CONTENT_MAP["booking-links"]);
  uploadContent(constants.CONTENT_MAP["booking-links"], bookingRows, "booking-links");

  const contentRows = prepareUpdateFile("content.csv", constants.CONTENT_MAP["content"]);
...
function uploadContent(fileConstants: ConstantsMap["content"] | ConstantsMap["booking-links"], rowObjects: Row[], contentType: "content" | "booking-links") {
  const currentDynamicContent = getFileJson(constants.CONTENT_TARGET);
