...
test("getEmailBody renders oath email correctly", () => {
  const emailBody = emailService.getEmailBody({ fields: allOtherFields, payment: paymentViewModel, reference: "1234" }, "affirmation");
...
test("getEmailBody renders cni template correctly", () => {
  const emailBody = emailService.getEmailBody({ fields: allOtherFields, payment: paymentViewModel, reference: "1234" }, "cni");
  expect(emailBody).toContain("<li>First name: foo</li>");
...
test("getEmailBody handles a cni postal form properly", () => {
  const emailBody = emailService.getEmailBody({ fields: allOtherFields, payment: paymentViewModel, reference: "1234", postal: true }, "cni");
  expect(emailBody).toContain("<li>First name: bar</li>");
});

test("getEmailBody handles a cni form with postal set to false properly", () => {
  const emailBody = emailService.getEmailBody({ fields: allOtherFields, payment: paymentViewModel, reference: "1234", postal: false }, "cni");
  expect(emailBody).toContain("<li>First name: foo</li>");
  expect(emailBody).toContain("notice of marriage and marital status affirmation");
});
